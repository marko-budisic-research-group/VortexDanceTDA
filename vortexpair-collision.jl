...
########################################################
 ...
 for i = 1:2
    for j = 1:noICs
        ...
        c = round.((2 .* rand(Float64, 8) .- 1)./ 1.5, digits=1) 
        groups = 4
        noVortices = randomSetSum(10, groups)

        # 1. Define initial configuration of vortices.
        x1, y1, γ1 = vortex_group( noVortices[1], (c[1], c[2]), -1.0, 0.1, 0.05 )
        x2, y2, γ2 = vortex_group( noVortices[2], (c[3], c[4]), 1.0, 0.1, 0.05 )

        x3, y3, γ3 = vortex_group( noVortices[3], (c[5], c[6]), 1.0, 0.1, 0.1 )
        x4, y4, γ4 = vortex_group( noVortices[4], (c[7], c[8]), -1.0, 0.1, 0.1 )
